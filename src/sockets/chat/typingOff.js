...
let typingOff=(io)=>{
	...
	io.on("connection",(socket)=>{  // lang nghe su kien connection khi user mo 1 tab mo
		...
		socket.on("user-is-not-typing",(data)=>{  //lang nghe su kien add-new-contact tu user
			if(data.groupId){
				let response={
				...
				if(client[data.groupId]){
					client[data.groupId].forEach(socketId=>{
						io.sockets.connected[socketId].emit("response-user-is-not-typing",response); //gui ve su kien response-add-new-contact cho client
					})
				}
			}
			if(data.contactId){
