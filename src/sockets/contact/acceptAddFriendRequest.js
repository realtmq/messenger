...
let acceptAddFriendRequest=(io)=>{
	...
	io.on("connection",(socket)=>{  // lắng nghe sự kiên khi user mở 1 tab mới
		...
		socket.on("accept-add-friend-request",(data)=>{ //bắt sự kiện "delete-addfriendrequest",kèm id người gửi lời mời kết bạn
			let currentUser={
		 		id:socket.request.user._id,  //lấy thông tin của người nhận lời mời kết bạn(là socket gửi sự kiện delete-add-friend-request)
			...
			if(client[data.contactId]){
				client[data.contactId].forEach(socketId=>{ //lấy tất cả socket id của người gửi lời mời kết bạn  
					io.sockets.connected[socketId].emit("response-accept-add-friend-request",currentUser);//gửi về id của người nhận lời mời kết bạn
				});
			}
		}); 
...
module.exports=acceptAddFriendRequest